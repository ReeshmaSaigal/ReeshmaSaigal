...
namespace Domain.Service.JobProvider
{
  ...
  /*      public async Task<JobPost> UpdateAsync(JobPost Updatedjob, Guid id)
        ...
        {
            return await _context.JobPosts.FirstOrDefaultAsync(j => j.Id == jobId);
        }
        ...
        public async Task<SignUpRequest>GetSignUpRequestByEmailAsync(string email)
        {
            return await _context.SignUpRequests.FindAsync(email);
        }
